...
//         console.log("Product being added to cart:", product); // Debugging
...
//             return; // Prevent further execution if product is invalid
//         }
...
//     //     try {
//     //         const response = await axios.post(
//     //             "http://localhost:5500/cartRouter/order",
//     //             { ...form, orders: carts },
//     //             { headers: { Authorization: localStorage.getItem("token") } }
//     //         );
//     //         if (response.status === 200) {
...
//     return (
//         <CartContext.Provider value={{ cartlistCount, setCartlistCount, addToCart, removeFromCart, fetchCart }}>
//             {children}
...
const CartContextProvider = ({ children }) => {
    const [cartlistCount, setCartlistCount] = useState(0);
    const [carts, setCarts] = useState([]); // Ensure this is defined
    ...
    return (
        <CartContext.Provider value={{ cartlistCount,setCartlistCount, addToCart, isLoggedIn,removeFromCart, setIsLoggedIn, carts }}>
            {children}
        </CartContext.Provider>
    );
};
