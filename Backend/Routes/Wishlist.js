...
const { Wish } = require("../Modal/WishModal");
...
wishRouter.post("/addwish", Auth, async (req, res) => {
    const { price, category, image, productId,discountPrice,name,description } = req.body;
    const userId = req.userId;


    if (!price || !discountPrice || !category || !image || !productId || !name || !description) {
        return res.status(400).json({ msg: "Missing required fields", fields: { price, category, image, productId,discountPrice,name,description } });
    }
